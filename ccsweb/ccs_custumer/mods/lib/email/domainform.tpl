 ...
 * Version:      $Id: domainform.tpl 1562 2007-06-26 21:43:38Z ckb6 $
 *
...
<tr>    
    ...
    <td>
    {if $action == "edit"}
    <input type="hidden" name="customer_id" value={$domain.customer_id}>
    <input type="hidden" name="owner" value="{$domain.owner}">{$domain.owner}
    {else}
    <select name="customer_id" id="customer_id">
    {foreach from=$customers item=customer}
        <option value="{$customer.customer_id}" {if $customer.customer_id == $domain.customer_id}selected="selected"{/if}>{$customer.username}@{$customer.domain}</option>
    {/foreach}
    {/if}
    </select>
    </td>
