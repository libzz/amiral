...
<div id="lblogin">
{if $session_state != "NO"}
Logged in to {$network_name} as:&nbsp;<b>{$username}</b>&nbsp;
{if $changeset > 0}
...
{if $session_state != "NO"}
({$session_state|upper} Session)
{/if}
</div>
<div id="lbbanner">
{if $online == "yes" && $changeset > 0}
    {if $changesets > 0}
        Changeset #{$changeset} Open ({$changesets} concurrent changesets)
    {else}
        Changeset #{$changeset} Open
    {/if}
{else}
    {if $nsessions > 0}{$nsessions}{else}No{/if} Active Sessions
{/if}
</div>
&nbsp;
{if $update!=1}
