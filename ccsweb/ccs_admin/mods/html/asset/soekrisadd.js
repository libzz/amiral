 ...
 *
 * Asset Management Interface - Soekris Bulk Asset Addition
 * 
...
var myrules = {
	...
	'#no_soekris' : function(element){
        element.onchange = noSoekrisChanged;
    },
};
...
function enableForm(asset_type_id)
{
    ...
    addSoekrisRows($F("no_soekris"));
    Behaviour.apply();

}
function addSoekrisRows(n)
{
    ...
    for (i=1;i<=n;i++) {
        ...
        row.appendChild(mac);
        tbl.appendChild(row);
        setError("isoekris_" + i, "You must specify a serial number or " +
            "MAC address!");
    }
    Behaviour.register(myrules);

}
function noSoekrisChanged() 
{
    ...
    if (n == 0) {
        alert("Must be at least 1 soekris!");
        $("no_soekris").value = "1";
        return;
    }
...
function validateSoekris(e)
{
    ...
    if (serial != "" || mac != "") {
        ...
        setError(parts[0], "You must specify a serial number or " +
            "MAC address!");
    }
