...
#
# Provides session and login management functionality for the configuration
# system.
#
...
# This file is part of crcnetd - CRCnet Configuration System Daemon
#
# Module initialisation code for the CRCnet Configuration System.
# Provides basic startup code to load the required daemons
#
# Author:       Matt Brown <matt@crc.net.nz>
# Version:      $Id$
#
...
from ccsd_config import config_get, config_getboolean
from ccsd_events import catchEvent
from ccsd_server import ccsd_xmlrpc, exportViaXMLRPC
from crcnetd.version import ccsd_version
...
class ccsd_session_error(ccsd_error):
    pass
...
class ccsd_session:
    ...
    def __init__(self, login_id, mode, token="", initiated=-1, expires=-1, \
        ...
        if sid==-1:
            ...
            if session is None:
                raise ccsd_session_error("No admin session!")
            ...
            if res == -1:
                raise ccsd_session_error("Unable to allocate session ID")
    ...
    def _connect(self):
        ...
        # Setup a database connection  
	log_info("Setup a database connection")
        self.db = PgSQL.connect(host=self.dhost, database=self.database, \
    ...
    def getSessionObject(self):
        ...
        if self.is_invalid == 1:
            raise ccsd_session_error("Invalid session object!")
    ...
    def begin(self, description="", initiator="", implicit=0):
        ...
        if self.changeset != 0:
            self.lock.release()
            raise ccsd_session_error("There is already a changeset open!")
        ...
        try:
            ...
            self.lock.release()
            raise ccsd_session_error("Could not create changeset!")
        ...
        if len(res) != 1:
            ...
            # Raise error
            self.lock.release()
            raise ccsd_session_error("Could not retrieve changeset id!")
        ...
        if not implicit:
            try:
                from ccsd_cfengine import ccs_revision
                self.revision = ccs_revision(self, self.changeset)
            except:
    ...
    def commit(self, description=""):
        ...
        try:
            ...
            self.lock.release()
            raise ccsd_session_error("Could not commit changeset!")
    ...
    def query(self, sql, params, icursor=None):
        ...
        if not sql.lower().startswith("select"):
            self.lock.release()
            raise ccsd_session_error("Queries must begin with select!")
        ...
        try:
            ...
            self._connect()
            self.lock.release()
            raise ccsd_session_error("Unable to obtain cursor for query!")
        ...
        try:
            ...
            if isinstance(value, str) and value.startswith("no connection"):
                self._connect()
            self.lock.release()
            raise ccsd_session_error(errStr)
    ...
    def execute(self, sql, params):
        ...
        if self.changeset > 0 and self.changesetOpen==0:
            self.lock.release()
            raise ccsd_session_error("Session is currently completing a " \
                    "changeset. No modifications to database allowed!")
    
        # Return now for queries that are a select
        if sql.lower().startswith("select"):
            self.lock.release()
            return ccsd_session_error("Select queries cannot be executed. " \
                    "Use query instead.")
        ...
        if self.changeset == 0 and require_changeset:
            self.begin(implicit=1, initiator="ccsd_session.execute" , \
        ...
        try:
            ...
            self.lock.release()
            raise ccsd_session_error("Could not obtain cursor")
...
class ccsd_basic_session(ccsd_session):
    ...
    def getSessionObject(self):
        ...
        dict = ccsd_session.getSessionObject(self)
        dict["basicSession"] = True
        return dict
        
    def getCookieToken(self):
        raise ccsd_session_error("Basic sessions do not support cookie tokens")
...
def loadSessions():
    ...
    for row in res:
        try:
            session = ccsd_session(row["login_id"], \
...
def getSession(session_id):
    ...
    if session_id not in ccsd_session.sessions.keys():
        return None
    
    return ccsd_session.sessions[session_id]
...
def getSessionE(session_id):
    ...
    if session_id not in ccsd_session.sessions.keys():
        raise ccsd_session_error("Session does not exist!")
...
def isSessionValid(sauth):
    ...
    # Validate parameters
    if type(sauth) != type({}):
        raise ccsd_session_error("Invalid session dictionary!")
    ...
    if sauth["session_id"] <= 0:
        raise ccsd_session_error("Invalid session ID")
    
    # Validate the session parameters
    if sauth["session_id"] not in ccsd_session.sessions.keys():
        raise ccsd_session_error("Session does not exist")
    session = ccsd_session.sessions[sauth["session_id"]]
    if session.login_id != sauth["login_id"]:
        raise ccsd_session_error("Session login_id does not match!")
    if session.token != sauth["token"]:
        raise ccsd_session_error("Session token is invalid!")
    
    # Update the expiry time
    session.updateExpiry()
    ccsd_session.sessions[sauth["session_id"]] = session
...
def sessionMaintenance(*args, **kwargs):
    ...
    try:
        ...
        for sessionID,session in ccsd_session.sessions.items():
...
def _validateCustomerPassword(username, password):
    ...
    from crcnetd.modules.ccs_contact import getCustomerCache
...
def startSession(login_id, mode):
    ...
    for sess_id, sess in ccsd_session.sessions.items():
...
def startBasicSession(login_id, mode):
    ...
    """
    newsession = ccsd_basic_session(login_id, mode)
    obj = newsession.getSessionObject()
    ccsd_session.sessions[obj["sessionID"]] = newsession
...
def customer_login(username, password):
    ...
    #    raise ccsd_session_error("Username too long, must be < 16 characters!")
    if len(password) <= 0:
        raise ccsd_session_error("Password too short, must be > 0 characters!")

    # Check username / password
    login_id =  _validateCustomerPassword(username, password)
    if not login_id:
        raise ccsd_session_error("Invalid username or password")
...
def getNoChangesets(session_id):
    ...
    count = 0
    for session_id2,tsession in ccsd_session.sessions.items():
...
@exportViaXMLRPC(SESSION_NONE, AUTH_NONE, asynchronous=True)
def getNoSessions():
    return len(ccsd_session.sessions)

@exportViaXMLRPC(SESSION_NONE, AUTH_NONE, asynchronous=True)
def getVersion():
    """Returns a version string """
    
    return "CRCnet Configuration System Daemon %s" % (ccsd_version)
