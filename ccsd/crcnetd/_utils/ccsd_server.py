...
from ccsd_daemon import shutdownHandler
from ccsd_config import config_get, config_getboolean, config_getint
from ccsd_events import registerEvent, triggerEvent
...
def _registerResource(root, desc, path, res_class, **kwargs):
    ...
    if path.startswith("/"):
        raise ccs_server_error("Path must not begin with /!")
    
    # Check if the path is already registerd
    paths = root.listNames()
    if path in paths:
        raise ccs_server_error("Path already registered!")
...
def stopCCSDServer():
    """Stops the CCSD Server by killing the twisted mainloop."""
...
class ccsd_xmlrpc(xmlrpc.XMLRPC):
    """The CCSD XMLRPC server. Exposes methods to the world."""
    ...
    def exportViaXMLRPC(mode, group, classmethod=False, name="", 
        ...
        def decorator(func):
            ...
            if efunc["name"] in ccsd_xmlrpc._functions.keys():
                raise ccs_server_error("Duplicate XMLRPC function registered!")
            ...
            if classmethod:
                ...
                efunc["function"] = func
            # Store it
            ccsd_xmlrpc._functions[efunc["name"]] = efunc
    ...
    def updateClassMethod(func, name=""):
        ...
        if name not in ccsd_xmlrpc._functions.keys():
            raise ccs_server_error("Cannot update unregistered method!")
        if ccsd_xmlrpc._functions[name]["function"] is not None:
            raise ccs_server_error("Class method already registered!")
        
        # Store the new function
        ccsd_xmlrpc._functions[name]["function"] = func
        ...
        * Dispatches requests to the appropriate function that was earlier
        ...
        try:
            ...
            reactor.callInThread(self._executeFunction, request, func, auth, \
                    *args)
        except ccs_server_error:
            (type, value, tb) = sys.exc_info()
            log_error("Call to %s failed! - %s" % (functionPath, value), \
                    (type, value, tb))
            self._cbRender(xmlrpclib.Fault(CCSD_CALLFAILED, value), request)
        except:
            # Unexpected error
            (type, value, tb) = sys.exc_info()
...
class ccsd_local_xmlrpc(ccsd_xmlrpc):
    ...
    def __init__(self, localnets):
        ...
        for net in localnets:
            try:
                ...
                ccsd_xmlrpc.localnets.remove(net)
                continue
    ...
    def render(self, request):
        ...
        if not self._isLocalIP(request.getClientIP()):
            ...
            return
        # Call parent
        return ccsd_xmlrpc.render(self, request)

# Bring exportViaXMLRPC,updateClassMethod into global module namespace
exportViaXMLRPC = ccsd_xmlrpc.exportViaXMLRPC
updateClassMethod = ccsd_xmlrpc.updateClassMethod

processedClasses = []
processedMethods = []
def processClassMethods(mod):
    """Handles registration for ccs_class derived classes in a module
    ...
    syms = dir(mod)
    for sym in syms:
        # Skip objects that are not derived from ccs_class
        cls = eval("mod.%s" % sym)
        if not inspect.isclass(cls):
            continue
        if ccs_class not in inspect.getmro(cls):
            continue
