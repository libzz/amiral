...
from crcnetd._utils.ccsd_server import exportViaXMLRPC, processClassMethods
from crcnetd._utils.ccsd_session import getSession, getSessionE

class ccs_service_error(ccsd_error):
    pass

class ccsd_service(ccs_class):
    ...
    def addService(self, host_id):
        ...
        from crcnetd.modules.ccs_host import validateHostId
    ...
    def getHostDetails(self, host_id):
        ...
        res = session.query("SELECT * FROM service_host WHERE " \
                "service_id=%s AND host_id=%s", (self.service_id, host_id))
        if len(res) < 1:
            raise ccs_service_error("Service is not configured on host!")
    ...
    def changeServiceHostState(self, host_id, state):
        ...
        if state!="t" and state!="f" and state!="":
            raise ccs_service_error("Invalid state value!")
    ...
    def initialiseService():
        ...
        """
        raise ccs_service_error("initialiseService cannot be called on " \
                "the ccsd_service class. You must override it")
...
def getServiceName(session_id, service_id):
    ...
    res = session.query(sql, (service_id))
    if len(res) != 1:
        raise ccs_service_error("Invalid service ID!")
...
def getServiceID(session_id, service_name):
    ...
    if len(res) != 1:
        raise ccs_service_error("Invalid service name!")
...
def validateServiceProp(session_id, propDetails, propValue, enabledState=False):
    ...
    if propDetails["required"] and enabledState:
        if len(propValue) <= 0 or propValue is None:
            raise ccs_service_error("Required Property!")
    if propDetails["prop_type"] == "number":
        try:
            n = long(propValue)
        except:
            raise ccs_service_error("Invalid numeric property!")
...
def getServiceInstance(session_id, name):
    ...
    if name not in _services.keys():
        raise ccs_service_error("Invalid service name! - %s" % name)
