...
DEFAULT_SITE_NAME = "Default CRCnet Configuration System Installation"
...
DEFAULT_CONFFILE = "/etc/ccsd/%s.conf" % os.path.basename(sys.argv[0])
DEFAULT_PIDFILE = "/var/run/%s.pid" % os.path.basename(sys.argv[0])
DEFAULT_CONFIG_SVNROOT = "/var/lib/svn/ccsd/configs"
DEFAULT_REQUEST_LOG = "/tmp/ccsd.log"
DEFAULT_SERVICE_LIB_DIR = "/usr/lib/ccsd/services"
DEFAULT_SERVICE_DATA_DIR = "/usr/share/ccsd/services"
DEFAULT_TMPDIR = "/tmp"
DEFAULT_PROFILE_DIR = "/tmp/ccsd-profile"
...
CCSD_ERROR = ERROR_BASE + 1

CCSD_BADPARAM = ERROR_BASE + 10

CCSD_DBERROR = ERROR_BASE + 20

CCSD_AUTHFAIL = ERROR_BASE + 30

CCSD_CALLFAILED = ERROR_BASE + 40
...
# Types of link
CCSD_LINK_ADHOC = "AdHoc"
CCSD_LINK_MANAGED = "Managed"
CCSD_LINK_ETHERNET = "Ethernet"
CCSD_LINK_QB20 = "QuickBridge20"
CCSD_LINK_TRANGO = "Trango"
...
# Base Classes
#############################################################################
class ccsd_error(Exception):
    def __init__(self, value):
        self.value = value
    def __str__(self):
        return repr(self.value)   
    
class ccs_class(object):
    """Provides generic functionality that all ccs classes will use
...
class CCSHTTPServer(HTTPServer):
...
class CCSTransport(xmlrpclib.Transport):
...
def getSoekrisMac(serial, iface=0):
    ...
    if serial < 0 or serial > 0xFFFFF:
        raise ccsd_error("Invalid soekris Serial number!")
    if iface < 0 or iface > 3:
        raise ccsd_error("Invalid interface number!")
...
def getSoekrisSerial(mac):
    ...
    if len(mac) != 17 or not \
        ...
        # Invalid MAC address
        raise ccsd_error("Invalid soekris MAC address")
